...
from mcp_src.rag_tool.rag.model import (DocumentLoader,
                 TextSplitter,
                 VectorStoreProvider,
                 SplitConfig,
                 RAGConfig,
...
def load_kb_file_registry() -> list[Dict]:
    ...
    try:
        ...
        log.warning(f"台账文件缺失或损坏，按空台账处理: {KB_FILE_REGISTRY_PATH}")
        return []

def save_kb_file_registry(registry: list[Dict]):
    """保存台账，同名file_md5会覆盖旧条目"""
    log.info(f"保存台账，总记录数：{len(registry)}, 文件列表: {registry}")
...
class RAGFactory:
    ...
    def __init__(self) -> None:
        ...
        self._store_cache: dict[str, VectorStoreProvider] = {}  # 新增：向量库实例缓存
    ...
    def create_store(self, name: str, config: Optional[Any] = None) -> VectorStoreProvider:
        """创建向量库。默认配置会缓存复用，避免工具调用在非主线程重复初始化死锁。"""
        if config is None and name in self._store_cache:
        ...
        if config is None:
            ...
            store = self._store_registry[name](config)
            self._store_cache[name] = store
            return store
    ...
    def _collect_supported_files(self, directory: Path) -> list[Path]:
        """扫描目录，返回受支持的文件列表；存在不支持后缀则报错"""
        files = []
        unsupported = []
        for file_path in directory.rglob('*'):
            if file_path.is_file():
                if file_path.suffix.lower() in self._loader_registry:
                    files.append(file_path)
                else:
                    unsupported.append(f"{file_path.name}（{file_path.suffix}）")
        if unsupported:
            raise RuntimeError("以下文件格式不受支持，无法入库：" + "；".join(unsupported))
        return files

    def _merge_registry(self, old_registry: list[Dict], new_entries: list[Dict]) -> None:
        """合并新旧台账并保存"""
        md5_map = {item['file_md5']: item for item in old_registry.copy()}
        for entry in new_entries:
            md5_map[entry['file_md5']] = entry
        save_kb_file_registry(list(md5_map.values()))
    ...
    def index_directory(
        ...
        files = self._collect_supported_files(directory)
        ...
        for file_path in files:
            ...
            try:
                if f_md5 in stored_file_md5:
                    log.info(f"完整文件副本已存在，跳过 {file_path.name}")
                    continue
                ...
                log.warning(f" 加载文件失败： {file_path}: {e}")
        ...
        self._merge_registry(load_kb_file_registry(), new_entries)

        log.info(f" 文本分块入库完成，目录： {dir_path} ")
        return store
    # ─── 检索 ────────────────────────────────────────────────────

...
from mcp_src.rag_tool.rag.factory import register_vectorstore
from mcp_src.rag_tool.rag.model import VectorStoreProvider, MilvusLiteVectorConfig
...
class MilvusLiteStore(VectorStoreProvider):
    ...
    def __init__(self, config: MilvusLiteVectorConfig) -> None:
        ...
        self._init_store()
        log.info(f"加载 Milvus 向量库: {self._uri}")
    ...
    def add_texts(self, texts: list[str], metadatas: Optional[list[dict]] = None,
        ...
        if not texts_list:
            log.warning("清洗后无有效内容，放弃入库")
            return
        # 云端 embedding 单次请求最多 20 条，分批写入（本地 embedding 也兼容）
        batch_size = 20
        total = 0
        for i in range(0, len(texts_list), batch_size):
            sub_texts = texts_list[i:i + batch_size]
            sub_meta = metadatas_list[i:i + batch_size] if metadatas_list else None
            self._store.add_texts(texts=sub_texts, metadatas=sub_meta, ids=None)
            total += len(sub_texts)
        log.info(f"Milvus 添加 {total} 条数据")
    ...
    def load(self, path: str) -> None:
        ...
        self._init_store()
        log.info(f"切换加载 Milvus 向量库: {self._uri}")

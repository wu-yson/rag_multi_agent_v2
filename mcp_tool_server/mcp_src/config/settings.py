...
class AppSettings(BaseSettings):
    ...
    # chroma向量库 __
    chroma_persist_path: Optional[str] = str(BASE_DIR /"mcp_src/db/cloud_chroma_db")  # 持久化路径
    chroma_embedding_model: Optional[str] = "qwen3-embedding:8b"  # 嵌入模型
    chroma_search_top_k: int = 4  # 搜索相似度检索返回条数

    # Milvus_Lite向量库 __
    milvus_persist_path: Optional[str] = str(BASE_DIR / "mcp_src/db/cloud_milvus_db")  # 持久化路径
    milvus_embedding_model: Optional[str] = "qwen3.7-text-embedding"  # 嵌入模型
    milvus_search_top_k: int = 4  # 搜索相似度检索返回条数


    # faiss向量库 __
    faiss_persist_path: str = str(BASE_DIR /"mcp_src/db/cloud_faiss_db")  # 持久化路径
    faiss_embedding_model: str = "qwen3-embedding:8b"  # 嵌入模型
    faiss_search_top_k: int = 4  # 搜索相似度检索返回条数
    faiss_meta_path: str = str(BASE_DIR / "mcp_src/db/cloud_faiss_db/file_meta.json")   # 元数据存储路径

...
from mcp_src.llm_tool.models import ProviderInitializationError, ProviderConfig, ModelNotSupportedError
...
class QWENProvider:
    ...
    def get_client(self, model_name: str) -> BaseChatModel | DashScopeEmbeddings:
        """ 获取模型客户端。"""
        if model_name not in self.supported_models_embed:
            raise ModelNotSupportedError(f"模型 {model_name} 不是通义模型, 仅支持向量模型: {self.supported_models_embed}")
    ...
    def get_client(self, model_name: str) -> BaseChatModel | OllamaEmbeddings:
        ...
        if (model_name not in self.supported_models_embed
            ...
            raise ModelNotSupportedError(
                f"模型 {model_name} 不是本地模型, 仅支持模型: {self.vision_models}, {self.supported_models_embed}")
        if model_name in self.supported_models_embed:
